...
public class SkillProfileControllerIntegrationTest {
    ...
    @DisplayName("POST " + PATH
            + " (mockmvc + db) create superhero skill profile fails when missing field")
        ...
        mockMvc.perform(
                        post(buildUri(1))
                                .content(superheroDtoSerializer.get().writeValueAsString(
                                        SkillProfileDto.fromDomain(failingSkillProfile)))
    ...
    @Test
    @DisplayName("POST " + PATH
            + " (mockmvc + db) create superhero skill profile fails when payload contains skill object")
    ...
    @Test
    @DisplayName("POST " + PATH
            + " (mockmvc + db) create superhero skill profile fails when missing payload")
    ...
    @Test
    @DisplayName(
            "POST " + PATH + " (mockmvc + db) create superhero skill profile fails when not found")
        ...
        MvcResult mvcResult = mockMvc.perform(
                        post(buildUri(1))
                                .content(superheroDtoSerializer.get()
                                        .writeValueAsString(SkillProfileDto.fromDomain(skillProfile)))
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                                .with(allAuthorities))
                .andDo(print())
    ...
    @DisplayName("PUT " + PATH
            + "/{skillId} (mockmvc + db) update superhero skill profile fails when missing field")
    ...
    @Test
    @DisplayName("PUT " + PATH
            + "/{skillId} (mockmvc + db) edit superhero skill profile fails when superhero not found")
    public void testUpdateSuperheroSkillProfileFailsWhenNoSuperheroFound() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(
                        put(buildUri(1) + "/" + 1)
                                .content(objectMapper.writeValueAsString(
                                        TestDataConfiguration.getSkillProfileToUpdateStub()))
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                                .with(allAuthorities))
                .andDo(print())
        ...
        assertThat(actual.getStatus()).isEqualTo(HttpStatus.NOT_FOUND.value());
        assertThat(actual.getError()).isEqualTo(HttpStatus.NOT_FOUND.getReasonPhrase());
        assertThat(actual.getMessage()).startsWith(
                SkillProfileService.SKILL_PROFILE_SUPERHERO_NOT_FOUND_MSG);
        assertThat(actual.getPath()).isEqualTo(buildUri(1) + "/" + 1);
    }

    @Test
    @DisplayName("DELETE " + PATH
            + "/{skillId} (mockmvc + db) delete superhero skill profile fails when not found")
    ...
    class NoDatabaseChangeTest {
        ...
        @DisplayName("GET " + PATH
                + " (mockmvc + db) list all superhero skill profile fails when not found")
        public void testListSuperheroSkillProfilesFailsWhenNoSuperhero() throws Exception {
            ...
            assertThat(actual.getError()).isEqualTo(HttpStatus.NOT_FOUND.getReasonPhrase());
            assertThat(actual.getMessage()).startsWith(
                    SkillProfileService.SKILL_PROFILES_SUPERHERO_NOT_FOUND_MSG);
            assertThat(actual.getPath()).isEqualTo(buildUri(1) + "");
        }
        ...
        @Test
        @DisplayName("GET " + PATH
                + "/{skillId} (mockmvc + db) show superhero skill profile fails when not found")
